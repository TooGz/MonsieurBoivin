---
title: "Atelier – Projets Maker, Bois & Électronique"
description: "Dans l’atelier : projets maker, électronique (ESP32, capteurs), bois et DIY. Journal de construction, tests matériels et expérimentations."
summary: "Projets maker, électronique et bois : découvrez les réalisations de l’atelier."
slug: "atelier"
layout: "list"
---

Bienvenue dans **l’atelier**.

Ici je documente mes projets maker :  
électronique (ESP32, capteurs), bois, impressions 3D et expérimentations diverses.

L’idée est simple :  
👉 construire, tester, comprendre — et partager tout le process.

---

## Projet en cours : Jeu de la Grenouille

Un projet complet mêlant :

- Conception bois
- Étude hardware
- Capteurs et détection
- ESP32 & programmation
- Journal de construction détaillé

➡️ **Voir le projet complet :**  
[Découvrir le Jeu de la Grenouille]({{< relref "atelier/grenouille/_index.md" >}})

---

## Électronique & ESP32

Si tu t’intéresses aux microcontrôleurs et aux capteurs :

- [ESP32 Pinout complet]({{< relref "posts/2025-11-04-esp32-pinout/index.md" >}})
- [Capteurs piézo & détection de vibration]({{< relref "posts/2025-10-21-capteurs-piezo-detection/index.md" >}})

Ces articles servent souvent de base aux projets plus avancés.

---

## Travail du bois

Dans certains projets, le bois est central :

- découpe
- assemblage
- finitions
- structure mécanique

Tu peux retrouver le détail des étapes dans les journaux de build des projets.

---

## À venir

D’autres projets sont en préparation :

- améliorations du jeu
- nouvelles expérimentations électroniques
- projets mêlant mécanique et microcontrôleurs

---

Si tu aimes comprendre comment les choses fonctionnent et suivre un projet de A à Z,  
tu es au bon endroit.
